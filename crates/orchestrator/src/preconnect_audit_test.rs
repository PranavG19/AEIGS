use crate::preconnect_audit::{
    PreconnectIssueKind, analyze_preconnects, preconnect_to_operations,
};
...
fn detects_http_preconnect() {
    ...
    assert!(issues.iter().any(|i| i.kind == PreconnectIssueKind::HttpOrigin));
}
...
fn detects_excessive_preconnects() {
    let links: Vec<String> = (0..8)
        .map(|i| {
            format!(
                r#"<link rel="preconnect" href="https://cdn{i}.example.com" crossorigin>"#
            )
        })
        .collect();
    let html = links.join("\n");
