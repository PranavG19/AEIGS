...
fn trusted_cdn_not_flagged_as_unknown() {
    let html = r#"<script src="https://cdnjs.cloudflare.com/ajax/libs/lodash.js" integrity="sha256-xxx"></script>"#;
    let issues = analyze_third_party_scripts(html, "example.com");
    assert!(
        !issues
            .iter()
...
fn with_sri_not_flagged() {
    let html =
        r#"<script src="https://cdn.jsdelivr.net/npm/vue@3" integrity="sha384-abc"></script>"#;
    let issues = analyze_third_party_scripts(html, "example.com");
    assert!(
        !issues
            .iter()
...
fn excessive_third_party_detected() {
    ...
    assert!(
        issues.iter().any(
            |i| matches!(i, ThirdPartyScriptIssue::ExcessiveThirdParty { count } if *count > 10)
        )
    );
}
...
fn severity_ordering() {
    assert!(
        third_party_script_severity(&ThirdPartyScriptIssue::HttpScript { url: "x".into() })
...
fn to_operations_count() {
    let issues = vec![
        ThirdPartyScriptIssue::TrackerScript { domain: "x".into() },
        ThirdPartyScriptIssue::HttpScript { url: "y".into() },
    ];
    let mut seq = 0;
