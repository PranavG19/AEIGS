...
fn apache_inode_etag_detected() {
    ...
    assert!(issues.iter().any(|i| matches!(i, EtagIssue::InodeLeak { .. })));
}
...
fn weak_etag_flagged() {
    ...
    assert!(issues.iter().any(|i| matches!(i, EtagIssue::WeakEtag { .. })));
}
...
fn long_etag_flagged() {
    ...
    assert!(issues.iter().any(|i| matches!(i, EtagIssue::LongEtag { length: 70, .. })));
}

#[test]
fn normal_length_etag_ok() {
    let issues = analyze_etag(Some(r#""abc123def456""#));
    assert!(!issues.iter().any(|i| matches!(i, EtagIssue::LongEtag { .. })));
}
...
fn timestamp_decimal_detected() {
    ...
    assert!(issues.iter().any(|i| matches!(i, EtagIssue::TimestampLeak { .. })));
}
...
fn sequential_small_number_detected() {
    ...
    assert!(issues.iter().any(|i| matches!(i, EtagIssue::SequentialEtag { .. })));
}
...
fn unquoted_etag_detected() {
    ...
    assert!(issues.iter().any(|i| matches!(i, EtagIssue::UnquotedEtag { .. })));
}
...
fn path_in_etag_detected() {
    ...
    assert!(issues.iter().any(|i| matches!(i, EtagIssue::InternalPathLeak { .. })));
}
...
fn corrupted_md5_detected() {
    ...
    assert!(issues.iter().any(|i| matches!(i, EtagIssue::HashMismatch { .. })));
}
...
fn display_inode_leak() {
    let issue = EtagIssue::InodeLeak { etag: "5f3a1b-264-5e8c4a0e".into() };
...
fn display_long_etag() {
    let issue = EtagIssue::LongEtag { etag: "x".repeat(70), length: 70 };
...
fn display_timestamp_leak() {
    let issue = EtagIssue::TimestampLeak { etag: "1700000000".into() };
...
fn display_internal_path_leak() {
    let issue = EtagIssue::InternalPathLeak { etag: "/var/www/x".into() };
    assert_eq!(issue.to_string(), "internal_path_leak: /var/www/x");
}

#[test]
fn display_hash_mismatch() {
    let issue = EtagIssue::HashMismatch { etag: "badhash".into() };
...
fn severity_inode_leak() {
    assert_eq!(etag_severity(&EtagIssue::InodeLeak { etag: String::new() }), 4.0);
}

#[test]
fn severity_weak_etag() {
    assert_eq!(etag_severity(&EtagIssue::WeakEtag { etag: String::new() }), 1.5);
}

#[test]
fn severity_long_etag() {
    assert_eq!(
        etag_severity(&EtagIssue::LongEtag { etag: String::new(), length: 70 }),
        2.5
    );
}
...
fn severity_timestamp_leak() {
    assert_eq!(etag_severity(&EtagIssue::TimestampLeak { etag: String::new() }), 3.0);
}

#[test]
fn severity_sequential_etag() {
    assert_eq!(etag_severity(&EtagIssue::SequentialEtag { etag: String::new() }), 3.5);
}

#[test]
fn severity_unquoted_etag() {
    assert_eq!(etag_severity(&EtagIssue::UnquotedEtag { raw: String::new() }), 1.0);
}
...
fn severity_internal_path_leak() {
    assert_eq!(
        etag_severity(&EtagIssue::InternalPathLeak { etag: String::new() }),
        4.5
    );
}
...
fn severity_hash_mismatch() {
    assert_eq!(etag_severity(&EtagIssue::HashMismatch { etag: String::new() }), 1.0);
}
...
fn operations_multiple_issues() {
    let issues = vec![
        EtagIssue::InodeLeak { etag: "a-b-c".into() },
        EtagIssue::WeakEtag { etag: "a-b-c".into() },
        EtagIssue::LongEtag { etag: "x".repeat(70), length: 70 },
    ];
    let mut seq = 0;
    let ops = etag_to_operations(&issues, &mut seq);
    assert_eq!(ops.len(), 3);
...
fn operations_confidence_is_half() {
    ...
    let ops = etag_to_operations(&issues, &mut seq);
    assert_eq!(ops.len(), 1);
    if let aegis_protocol::operation::GraphOperation::AddFinding {
        confidence, ..
    } = &ops[0].operation
    {
